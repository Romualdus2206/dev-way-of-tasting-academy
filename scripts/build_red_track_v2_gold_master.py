#!/usr/bin/env python3
"""Generate content/pipeline/RED_PIPELINE_CONTENT.md from ROOD_MODULES.md + premium data."""
...
SOURCE = ROOT / "content/archive/legacy/ROOD_MODULES.md"
OUT = ROOT / "content/pipeline/RED_PIPELINE_CONTENT.md"

DATA_MODULES = (
    "red_v2_gold_master_data_m1_m2",
    "red_v2_gold_master_data_m3_m5",
    "red_v2_gold_master_data_m6_m9",
)
...
def normalize_praktijk(praktijk: str, leerdoel: str) -> str:
    ...
    if re.search(r"\b(pair|eten|gerecht|menu|vlees|wild|kaas)\b", opdracht, re.I):
        items.append("Ik motiveerde de pairing via tannine-eiwit, vetbalans of umami")
    if not items:
        items = [
...
def build_header(raw: str) -> str:
    ...
    return (
        ...
        "**Bron:** `ROOD_MODULES.md` → "
        "`python3 scripts/build_red_track_v2_gold_master.py`\n\n"
        "---\n\n"
        f"{module_block}\n"
    )
...
def validate(text: str, repairs: dict[int, dict]) -> None:
    ...
    banned = r"NOTE:|TODO|FIXME|placeholder|repair|contentReference|oaicite|RED_MODULE"
