#!/usr/bin/env python3
"""Generate content/pipeline/ROSE_PIPELINE_CONTENT.md from ROSE_MODULES.md + premium data."""
...
SOURCE = ROOT / "content/archive/legacy/ROSE_MODULES.md"
OUT = ROOT / "content/pipeline/ROSE_PIPELINE_CONTENT.md"
LESSON_COUNT = 43

DATA_MODULES = (
    "rose_v2_gold_master_data_m1_m2",
    "rose_v2_gold_master_data_m3_m5",
    "rose_v2_gold_master_data_m6_m9",
)
...
def normalize_praktijk(praktijk: str, leerdoel: str) -> str:
    ...
    if re.search(r"\b(pair|eten|gerecht|menu|vis|salade|barbecue)\b", opdracht, re.I):
        items.append("Ik motiveerde de pairing via zuur-vet, zoutbalans of textuurmatching")
    if not items:
        items = [
...
def build_header(raw: str) -> str:
    ...
    return (
        ...
        "---\n\n"
        "## Rosé Track V2 — Gold Master\n"
        f"### {LESSON_COUNT} lessen · 9 modules · unieke slugs\n\n"
        "**Versie:** V2 Gold Master (leidende versie)\n"
        "**Bron:** `ROSE_MODULES.md` → "
        "`python3 scripts/build_rose_track_v2_gold_master.py`\n\n"
        "---\n\n"
        f"{module_block}\n"
    )
...
def validate(text: str, repairs: dict[int, dict]) -> None:
    ...
    banned = r"NOTE:|TODO|FIXME|placeholder|contentReference|oaicite|# ROSE_MODULE_\d"
    ...
    if set(repairs.keys()) != set(range(1, LESSON_COUNT + 1)):
        missing = set(range(1, LESSON_COUNT + 1)) - set(repairs.keys())
    ...
    for m in re.finditer(r"^# LES (\d+) — ", text, re.M):
        ...
        if "## Pro insight" not in chunk:
            raise SystemExit(f"Build index {build_idx}: missing Pro insight")
        if build_idx >= 16 and "## Food pairing" not in chunk:
            raise SystemExit(f"Build index {build_idx}: missing Food pairing")
        ...
        if not praktijk or "### Opdracht" not in praktijk.group(1):
            if "### Examenonderdeel" not in (praktijk.group(1) if praktijk else ""):
                raise SystemExit(f"Build index {build_idx}: Praktijkopdracht incomplete")
        if "### Checklist" not in (praktijk.group(1) if praktijk else ""):
            if build_idx != LESSON_COUNT or "### Examenonderdeel" not in praktijk.group(1):
                raise SystemExit(f"Build index {build_idx}: missing Checklist")
        kc = re.search(r"## Kernbegrippen \(DB field\)\s*\n(.*?)(?=\n---|\Z)", chunk, re.S)
        if not kc:
            raise SystemExit(f"Build index {build_idx}: missing Kernbegrippen")
        ...
        if any(i.lower() in GENERIC_KERNBEGRIPPEN for i in items):
            raise SystemExit(f"Build index {build_idx}: generic kernbegrip: {items}")
        if build_idx == 15:
            quiz = extract_section(chunk, "Quiz")
            if len(re.findall(r"^### Vraag \d+", quiz, re.M)) != 10:
                raise SystemExit("Build index 15: expected 10 quiz questions")
        if build_idx == LESSON_COUNT:
            if chunk.count("### Examenonderdeel") < 3:
                raise SystemExit("Final exam: exam praktijk incomplete")
            quiz = extract_section(chunk, "Quiz")
            if len(re.findall(r"^### Vraag \d+", quiz, re.M)) != 20:
                raise SystemExit("Final exam: expected 20 quiz questions")
...
def build() -> str:
    ...
    while j < len(parts):
        ...
        if build_idx not in repairs:
            raise SystemExit(f"No repair data for build index {build_idx} ({header.strip()})")
        polished, trailer = rebuild_lesson(build_idx, header, body, repairs[build_idx])
