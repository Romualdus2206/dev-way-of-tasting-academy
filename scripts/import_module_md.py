#!/usr/bin/env python3
"""Parse Academy module markdown and emit lesson/quiz SQL fragments."""
...
def parse_module_text(text: str, default_module_slug: str = "intro") -> Dict:
    ...
    for m in re.finditer(r"# LES \d+ — (.+?)\n(.*?)(?=\n# LES \d+ —|\Z)", text, re.S):
        ...
        def section(name: str) -> str:
            names = SECTION_ALIASES.get(name, [name])
            for variant in names:
                sm = re.search(rf"## {re.escape(variant)}\s*\n(.*?)(?=\n## |\Z)", body, re.S)
                if sm:
                    return re.sub(r"\n---\s*$", "", sm.group(1).strip())
            return ""
        ...
        feedback_m = re.search(r"## Quiz-feedback\s*\n+(.+?)(?=\n## |\Z)", body, re.S)
...
def parse_module_md(path: Path, default_module_slug: str = "intro") -> Dict:
...
if __name__ == "__main__":
    ...
    print(emit_lessons_sql(data["lessons"], "intro-port", "port"))
